...
void CMQTTClient::Connect (boolean bUseSSL,
			   const char *pHost, u16 usPort, const char *pClientIdentifier,
			   ...
			   const u8 *pWillPayload, size_t nWillPayloadLength)
{
	...
	else
	{
		...
		if (   pSSLSocket->AddCertificatePath ("/") < 0
		    || pSSLSocket->Setup (pHost) != 0)
		{
			CLogger::Get ()->Write (FromMQTTClient, LogError, "SSL setup failed");

			delete m_pSocket;
			m_pSocket = 0;

			OnDisconnect (MQTTDisconnectSSLSetupFailed);

			return;
		}
	}

	int nResult = m_pSocket->Connect (IPServer, usPort);
	if (nResult != 0)
	{
		CLogger::Get ()->Write (FromMQTTClient, LogError, "Cannot connect: %s (%d)", pHost, nResult);
